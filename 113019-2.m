...
%Uncomment to view some lovely pixel histograms to study intensitites
histR = histogram(RC, 100);
histG = histogram(GC, 100);
histB = histogram(BC, 100);
...
%Want to save the data? No worries - keep this uncommented
centroid_x = stats.Centroid(:,1);
centroid_y = stats.Centroid(:,2);
area = stats.Area 
t = table(centroid_x, centroid_y, area)
writetable(t,'red_centroid_output.csv');
...
area = stats.Area
t = table(centroid_x, centroid_y, area)
writetable(t,'green_centroid_output.csv');
...
area = stats.Area 
t = table(centroid_x, centroid_y, area)
writetable(t,'blue_centroid_output.csv');
